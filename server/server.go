...
func main() {
    ...
    http.Handle("/query", withCORS(withUserIDContext(srv)))
...
// withCORS разрешает запросы с любого источника (для теста)
func withCORS(h http.Handler) http.Handler {
	    return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		            w.Header().Set("Access-Control-Allow-Origin", "*")
			            w.Header().Set("Access-Control-Allow-Methods", "POST, GET, OPTIONS")
				            w.Header().Set("Access-Control-Allow-Headers", "Content-Type, X-User-Id, X-Yandex-Api-Key, X-Only-Approved, X-Projects-Filter")
