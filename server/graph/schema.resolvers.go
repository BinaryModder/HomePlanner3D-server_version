...
func (r *mutationResolver) CreatePlanningProject(ctx context.Context, input model.PlanningProjectInput) (*model.PlanningProject, error) {
    ...
    if input.Geometry != nil {
        ...
        for _, r := range input.Geometry.Rooms {
            ...
            if r.ID != nil { sb.WriteString(*r.ID) }
            sb.WriteString(" ")
    ...
    if input.Walls != nil {
        lb := 0
        for _, w := range input.Walls { if w.LoadBearing { lb++ } }
    ...
    if input.Constraints != nil {
        if len(input.Constraints.RegionRules) > 0 { sb.WriteString("; Правила: "); sb.WriteString(strings.Join(input.Constraints.RegionRules, ",")) }
